...
def openimage():
    ...
    global fn
    fileName = askopenfilename(initialdir='D:/PCOS 100% Code/test_set',
                               filetypes=[("all files", "*.*")])
    IMAGE_SIZE=200
    imgpath = fileName
    fn = fileName
...
def generate_pdf():
    ...
    if fn != "":
        ...
        c.drawString(100, 750, "Machine Learning Approaches On Polycystic Ovary Syndrome")
        c.drawString(100, 730, "Image Path: {}".format(fn))
        
        if fn != "":
            prediction = test_model_proc(fn)
            if prediction == "Polycystic Ovary Syndrome Detected":
                result_text = "PCOS Detected"
            else:
                result_text = "PCOS Not Detected"
            c.drawString(100, 710, "Prediction: {}".format(result_text))

        # Add more information as needed
        c.save()
        update_label("PDF Generated: output.pdf")
    else:
...
button6 = tk.Button(frame_alpr, text="Generate PDF", command=generate_pdf, width=15, height=1, font=('times', 15, ' bold '), bg="white", fg="black")
